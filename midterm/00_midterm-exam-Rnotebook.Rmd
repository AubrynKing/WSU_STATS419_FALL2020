...
library(devtools);
library(maps) #needed this for something
library(usethis) #also needed to isntall
library(rlang) # this too
...
library(parallel);
parallel::detectCores(); # 8 threads for me # Technically, this is threads, I have an 8-core processor 
...
seed = 5
  set.seed(seed) #
sprintf("seed = %d", seed) #there is no way to retrieve what the seed it in R
x.rnorm = rnorm(n=9999) #returns set of numbers
...
plot(x.rnorm, x.runif); #plot(x,y)
plot(x.runif, x.runif); #plot(x,y)

#sample(): randomly reorders the elements passed in
...
- Describe what each function `rnorm` and `runif` does. How are they similar?  How are they different?  
  ...
  rnorm() and runif() generate a set of numbers that follow the distribution
  of the function. rnorm() follows a normal distribution, and runif() follows
  a uniform distribution. 
  
  When the normal distribution is plotted against the frequency or probability
  of those observations , it has a symmetrical bell shape centered about the
  mean. This is where the majority of the observations fall, and as we get 
  further and further from the mean there are less and less observations, so
  the height of the frequency bars are lower. We can see this by looking at 
  the histogram. 
  
  In a uniform distribution ,the points are evenly distributed between two 
  end values. In R, the default end values are 0 and 1. By looking at the
  histogram we can see that the height of the frequency bars are roughly the
  same. This means that there are about the same number of points in each box
  between 0 and 1, it is, in other words, uniformly distributed. 
  
  The value for n that we pass in to rnorm() and runif() is the number of 
  observations that we want generated. When we have a larger number, the 
  patterns and shape of the distribution will become more clear to see. 
  
  
- What does the `sample` function do?

  The sample() function produces a list of random numbers according to the 
  parameters you give it. This includes the range of the numbers you want, the 
  number of elements you want, and if you want replacement. It is used to 
  generate random numbers. 

  The sample() function also randomly reorders the elements that you pass in
  to it. If we were to plot x.rnorm against x.rnorm, we would get the line 
  Y=x, but  when we plot x.rnorm against sample(x.rnorm), we are 
  reshuffling the points along the y axis. This can help us see the 
  distribution of the data. 
  
  For example, the plots of plot(x.rnorm,sample(x.rnorm)) and 
  plot(x.runif, sample(x.runif)) highlight the different shapes  of
  the distributions. 
...
- How was `plot(x.rnorm, x.runif);` different from `plot(x.rnorm, ( x.rnorm.sample = sample(x.rnorm) ) );` and `plot(x.runif, ( x.runif.sample = sample(x.runif) ) );`?  How would you describe the shape of each of these plots?
  ...
  plot(x.rnorm,sample(x.rnorm)): We can see how there is a high concentration 
  around the mean of the data (0), and there are less and less points the 
  further we get from the mean. I would describe this as a a fuzzy oval plot. 
  
  plot(x.runif, sample(x.runif): The points look very evenly distributed, there
  are no areas that are very sparse, or with any more points than any other. 
  This is exactly what a uniform distribution is, uniform. I would describe
  this plot as an evenly covered plot. 
...
## your code goes here ...
#monte solution from discussion

#rbinom(n, size, prob)
nsim=0
x = numeric(length =0) #vector
i=1
while(length(x)<5)
{
  ...
  if(result ==555)
  {
      ...
      nsim = nsim+1
      x = c(x, i)
      print(sprintf("seed = %d", i))
  }#end if
  i=i+1
  
}#end while
sprintf("The number of seeds is: %d", length(x))
sprintf("number of simulations: %d", nsim)
print(paste(c("The seeds are: ", x, sep = " "), collapse = " "))


#check 
print("Checking that result =555 with every seed")
for(seed in x)
{
  set.seed(seed)
  result = rbinom(n=1, 1000, 0.5)
  print(sprintf("result = %d", result))
}
...
#sub.myrolls = subsetDataFrame(myrolls, "roll.total", "==", 23); #could not find function "trimMe"
sub.myrolls = myrolls[which(myrolls$roll.total == 23),] #rows where roll total == 23, and all columns
...
There are 36 ways that you can roll a 23 form three dice at the same time.
The probability that you rill a 23 on a single throw is 0.036. 
</pre>
...
diceroll12= myrolls[which(myrolls$roll.total == 12),] 
outcomes12 = outcomes.df[which(outcomes.df$roll.total ==12),]


diceroll22= myrolls[which(myrolls$roll.total == 22),] 
outcomes22 = outcomes.df[which(outcomes.df$roll.total ==22),]
prob12or22 = outcomes12[1,'prob'] + outcomes22[1,'prob']
prob12or22
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- WRITE SOMETHING HERE --
The probability that you roll a 12 or a 20 on a single throw is 0.1. 
</pre>
...
## your code goes here ... if necessary
diceroll26= myrolls[which(myrolls$roll.total == 26),] 
diceroll26
outcomes26 = outcomes.df[which(outcomes.df$roll.total ==26),]



diceroll29= myrolls[which(myrolls$roll.total == 29),] 
outcomes29 = outcomes.df[which(outcomes.df$roll.total ==29),]
prob26or29 = outcomes26[1,'prob'] + outcomes29[1,'prob'] #0.015 + 0.003
prob26or29
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- WRITE SOMETHING HERE --
The probability that you will roll a 26 or a 29 on a single throw is 0.018.
</pre>
...
## your code goes here ... if necessary
#rolling a number twice: P1 * P2
diceroll3= myrolls[which(myrolls$roll.total == 3),] 
outcomes3 = outcomes.df[which(outcomes.df$roll.total ==3),]
pr3 = outcomes3[1,'prob']
outcomes3
pr3*pr3
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- WRITE SOMETHING HERE --
The probability of rolling a three twice in a row is 0.000001
</pre>
...
for(i in totals)
{
  ...
  prRoll = outcome[1,'prob']
  pr12OrLower = pr12OrLower +prRoll
}
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- WRITE SOMETHING HERE --
The probability of rolling a twelve or lower is 0.22.
</pre>
...
## your code goes here 
graphics::hist(jobs$job.count)
graphics::boxplot(jobs$job.count)
...
[What does the histogram tell you about the data?  What does the boxplot tell you about the data?  What does `summary` tell you about the data?]
  ...
  The histogram tells us about the distribution of the data. It can show us the
  minumum and maximum values, the outliers, where most of the data is 
  concentrated, the possible  distribution (uniform, normal, poisson, etc). The
  bins that are the most common will be the highest, and the outliers will 
  mostlikely be small and be far away from all the other bars. If there are a 
  lot of outliers on one side, we know that that is the direction that the data
  is skewed. 
  
  The boxplot tells us basically the same information in a different way. A 
  boxplot shows us the median, upper and lower quartiles, the 'min' and 'max'
  (which are the quarters +/- 1.5 times the interquartile range), and the 
  outliers, which are any poins outside the min and maximum. We can tell from 
  the width of the box where the numbers are concentrated, becasue that box or 
  line will be shorter. A long string of outliers past the min or max will tell
  us tke skew of the data, whether it is skewed right or left/ negatively or 
  positively. 
  
  The summary of the data tells us just the min. 1st quartile, median, mean, 
  3rd quartile, and max. It can tell us how the data is skewed, if there is a 
  large difference between the median and mean for example, or if there are 
  very uneven intervals between the 1st quartile, mean, and 3rd quartile. The 
  mean is very important to know, because it is a parameter for 
  many distributions, like normal distribution, and what we use to standardize 
  data points. The most important thing it tells us is about the spread of the 
...
[What does the histogram tell you about the data?  What does the boxplot tell you about the data?]
  ...
  The histogram can tell us that there are a handful of words whose jobcount is
  over 100,000. These are the skills that might be seen as basic, and I should
  be sure to know. The most common job count for different job words is around
  25,000, so for about 40 different job words, there are about 25,000 jobs each
  featuring that word, which means that being proficient at  a good handful
  from tha bundle of words will open up thousands of job opportunities. We can 
  tell that this data is skewed right, which is obvious because there can be no
  negative job counts
...
[What is your initial "perspective" of this data, now that you see it?  Why are the lines "parallel-ish"?  What kind of a trend is that? 
  ...
  The lines are parallelish, which means that the words are increasing in their
  frequency at roughly the same rate. This means that none of the words are 
  big buzz words that are just overtaking the data analysis job market. 

Now comment on the line of data for "Data analysis".  How is it trending?  How does it compare to other Search-Query Words?
  
  Data analysis is the top word. It is the most popular search term in the data
  analysis job market. It is increasing its overall count, but the other terms 
  are also increasing their overall counts at about the same rate. This is only
  four weeks of data, and we might get a different idea of which words are 
  slowly converging if we had a full years worth of data or at least several 
  months, because this is too short of a time frame to tell if, for example, 
  data analytics and machine learning are converging. 

What is your first perspective?
  My first perspective is that the missing data from git is super obvious. It 
  is the most striking feature from the graph. There is no logical reason why 
  there would be a git purge one week and all postings with he word git are
  deleted than brought back the next week. It must be missing data. It is an 
  obvious sign of missing data and whoever wrote the web scraping code should 
...
[Is this data missing or did it just drop to zero that week?  How does this relate to the other data (remember the idea of "continuity" in mathematics)?  What should you do about it?]
  ...
  It doesn't make sense for the data to be zero. That would have meant that
  thousands of companies posting a job that had the word git in it removed that
  posting for one week, for some reason, then put them all back up the
  next week. 
