...
# Choose (3) None to minimize headaches .... OOPS I chose yes
...
# rnorm(n, mean, var) generates multivariate random normal variate sets 
...
```{r, chunck-student-t}
#t.test(x.0.1, x.0.1b, paired=FALSE, var.equal=FALSE);  #noththis
#t.test(x.0.1, x.0.1b, paired = FALSE, var.equal=TRUE);
#t.test(x.0.1, alternative ="less", paired = FALSE, var.equal = FALSE)
#t.test(x.0.1, alternative ="greater", paired = FALSE, var.equal = FALSE)
t.test(x.0.1, x.0.1b, alternative = "less", paired=FALSE, var.equal=FALSE);
t.test(x.0.2, x.0.3, alternative = "two.sided", paired=FALSE, var.equal=FALSE);
#t.test(x.0.1, x.3.1, alternative = "two.sided", paired=FALSE, var.equal=FALSE);
```
In the first, where we don't assume that the variances are equal, evn thoug they are, the t statistics are the same, but in the first there are fewer degrees of freedom, and a wider confidence interval. 
...
```{r, chunck-welch-t} 
#same mean, diff var, unpaired
t.test(x.0.1, x.0.1b, alternative = "two.sided", paired=FALSE, var.equal=FALSE);
```
...
One was standardized to the min and the other to the max, this means that they got standardized to a number that was about the same, just in opposite directions (becuase it has an independent normal distribution), so ine was correlated one way anf the other the other. 
...
## rank & votes & so on ... 
#imdb.rv = removeAllColumnsBut( imdb.df, #errorHere 
           # c("rank","year","minutes",
            #  "ratings","metacritic","votes","millions") );
imdb.rv = imdb.df [c("rank", "year", "minutes", "ratings", "metacritic", "votes", "millions")]
dim(imdb.rv);  # 100 movies

#imdb.rv = removeNAsFromDataFrame(imdb.rv, c("metacritic","millions") );
imdb.rv = na.omit(imdb.rv) #removes rows with NA anywhere
#imbd.rv <- imdb.rv[!(is.na(c(imdb.rv$rnor, imdb.rv))) | !(is.na(i$cfam)),]   #from github, solution to fix problem made my typo typo
...
##### rank vs. minutes 
It looks like there is a correlation. In particular the box on the top edge of the scatter plot visual, because of the
dimensions of the graph it is easy to see that there is a downward trend. It doesn't look too strong, and there looks 
to be quite a large spread, but the correlation is visible.  

This is comfirmed by the correlation matrix, the correlation between rank and minutes is -0.28. 
##### millions vs. year 

Looking at unadjusted millions, it looks like there is a bit of a positive correlation. There is a large variance, 
and not the pattern isn't very clear, but I am imagining what the trendline would look like if I were to draw it and 
I am imagining it sloping up. 

In the correlation matrix the correlation is 0.26, which confirms that there is a positive correlation. 

##### ratings vs. adjMillions2000 
Looking at the scatterplot, I didn't see a correlation. It looked pretty random and I didn't think see
enough of an increase in the pattern to tell that there is a correlation. In the bubble chart, there is a 
light blue circle in rank/adjMillions, so there is a small positive correlation which is around .1 to .2. 

The correlation matrix officially says that the correlation is 0.14, which may mean that the relationship 
...
cor(X, Y, method="pearson");  # default
cor(X, Y, method="kendall");
cor(X, Y, method="spearman"); #maybe use spearman because rank isn't continuous
...
ylab('Y') 
```
All of the p values in the correlation test are greater than 0.05, which is alpha at
confidence level 95%, which means that the correlation is not significant.

Looking at the scatter plots, it is difficult to see a visual trend, so I am not surprised that the
...
```
There still doesn't appear any visual correlation between the three variables, ratings, millions, and adj millions. The graph is super cool. On the second plot it was cool to line up the x and y.
#### Measure
...
measure.aek = read.csv("https://raw.githubusercontent.com/AubrynKing/WSU_STATS419_FALL2020/master/Project1/measure-b7ae3660cd0f700f69ad759867c79192.txt", header=TRUE, sep="|");
...
for(i in 1:n.rows)
  {  
  measure.row = measure.aek[i,];
  for(one in getOne)
    {
    nidx = getIndexOfDataFrameColumns(measure.aek, one);
    ...
    measure.aek[i,nidx] =  row.m;
    }
  }

str(measure.aek);

x = measure.aek$hand.length.left
y = measure.aek$height.NA
colnames(measure.aek)
#plot(x, y, main = "Hand length(left) and height", xlab= "left hand length", ylab = "height")
plot(measure.aek[, c("hand.length.left", "hand.width.left", "foot.length.left", "height.NA", "floor.hip.left", "floor.armpit.left", "hand.elbow", "arm.reach.left", "arm.span.NA")], col =ifelse(measure.aek$gender == 'F', "light blue", "green"), pch = 19) #c("dark green", "dark blue")[measure.aek$gender]) #[measure.aek$gender])

colnames(measure.aek)


measure = measure.aek[,c("head.height.NA", "hand.width.left", "elbow.armpit.left", "arm.span.NA", 
                      "floor.kneepit.left", "floor.armpit.left", "floor.hip.left", "height.NA", 
                      "age", "foot.length.left")]


round( cor(measure), digits=2);
plot(measure);
    symnum( cor(measure), 
            diag = TRUE,
            corr = TRUE,
...
The closest to correlation =0 is Y= tan(x), which has a correlation of 0.147.
